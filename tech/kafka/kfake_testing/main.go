...
var mode = flag.String("mode", "inject-error", "Control function mode: inject-error, observe, or sleep")
...
func demoInjectError() {
	...
	cl, err := kgo.NewClient(
		...
		kgo.DefaultProduceTopic("inject-error-kfake-topic"),
	...
	log.Println("inject-error: producing a record (first attempt will fail)...")
...
func demoObserve() {
	c, err := kfake.NewCluster(
		kfake.NumBrokers(1),
		kfake.SeedTopics(3, "observe-kfake-topic"),
	)
	if err != nil {
		log.Fatalf("unable to create cluster: %v", err)
	}
	...
	cl, err := kgo.NewClient(
		kgo.SeedBrokers(c.ListenAddrs()...),
		kgo.WithLogger(kgo.BasicLogger(os.Stdout, kgo.LogLevelInfo, nil)),
		kgo.DefaultProduceTopic("observe-kfake-topic"),
		kgo.ConsumeTopics("observe-kfake-topic"),
	)
	if err != nil {
		log.Fatalf("unable to create client: %v", err)
	}

	log.Println("observe: producing 10 records...")
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	for i := range 10 {
		r := &kgo.Record{Value: []byte(fmt.Sprintf("observer with testing kfake %d", i))}
		if err := cl.ProduceSync(ctx, r).FirstErr(); err != nil {
			log.Fatalf("unexpected produce error: %v", err)
		}
	}

	log.Println("observe: consuming records...")

	fs := cl.PollFetches(context.Background())
	if fs.IsClientClosed() {
		return
	}
	log.Printf("consumed %d records", fs.NumRecords())
	cl.Close()

	log.Println("\t request type counts:")
	mu.Lock()
...
func demoSleep() {
	...
	c.ControlKey(int16(kmsg.Fetch), func(kreq kmsg.Request) (kmsg.Response, error, bool) {
		log.Println("\t [control] fetch arrived, sleeping until produce completes...")

		c.SleepControl(func() {
			<-produced
		})

		log.Println("  [control] produce observed, waking fetch")
	...
	consumer, err := kgo.NewClient(
		kgo.SeedBrokers(c.ListenAddrs()...),
		kgo.ConsumeTopics("sleep-kfake-topic"),
		kgo.WithLogger(kgo.BasicLogger(os.Stdout, kgo.LogLevelInfo, nil)),
	)
	if err != nil {
	...
	go func() {
		...
		done <- result{n: f.NumRecords(), err: fErr}
	}()
	...
	producer, err := kgo.NewClient(
		kgo.SeedBrokers(c.ListenAddrs()...),
		kgo.DefaultProduceTopic("sleep-kfake-topic"),
		kgo.WithLogger(kgo.BasicLogger(os.Stdout, kgo.LogLevelInfo, nil)),
	)
	if err != nil {
	...
	log.Println("sleep: producing a record to wake the sleeping fetch...")
	...
	res := <-done
	if res.err != nil {
		log.Fatalf("unexpected fetch error from channel: %v", res.err)
	}
	log.Printf("fetch completed with %d records", res.n)
}
