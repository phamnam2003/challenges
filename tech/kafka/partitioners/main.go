...
var (
	...
	strategy    = flag.String("strategy", "sticky", "partition strategy: sticky, sticky-key, round-robin, least-backup, manual, uniform-bytes")
...
func main() {
	...
	default:
		log.Fatalf("unknown partitioner %s", *strategy)
	}
	...
	for i := range *numRecords {
		...
		cl.Produce(ctx, r, func(r *kgo.Record, err error) {
			...
			log.Printf("produced to partition %d (key=%s)\n", r.Partition, string(r.Key))
		})
	}
